...
def main():
    ...
    for i in may_contain_leak:
        for j in may_contain_leak:
            combinationofleaks.append([i, j]) 
    #PROBABILITYLIST IS THE PROBABILITIES THAT A COMBINATION OF CELLS BOTH CONTAIN A LEAK
    probabilitylist = []
    for i in combinationofleaks:
        probabilitylist.append(grid[i[0][0]][i[0][1]].probabilityofleak*grid[i[1][0]][i[1][1]].probabilityofleak)
    #THE ZIPPED LIST HAS COMBINATION OF CELLS AS WELL AS THE PROBABILITIES
    zipped_list = zip(combinationofleaks, probabilitylist)
    ...
    while grid[leakCella.r][leakCella.c].status != 'Bot':
        ...
        maxprob = 0
        #MAXPROB WILL EVENTUALLY BE THE COMBINATION OF CELLS WITH HIGHEST CHANCE OF 
        #CONTAINING BOTH OF THE LEAKS
        for i in zipped_list:
            if i[1] > maxprob:
                maxprob = i[1]
                rowtomoveto = i[0][0][0]
                coltomoveto = i[0][0][1]
        path = pathBFS(grid, (rob.r, rob.c), (rowtomoveto, coltomoveto))
        ...
        if grid[rob.r][rob.c].leak == False:
            ...
            #WE UPDATE THE ZIPPED LIST BASED ON IF THE BOT CELL CONTAINS A LEAK
            #IT TAKES OUT THE CELLS ASSOCIATED WITH THE BOT CELL LOCATION
            probabilitymatrix[rob.r][rob.c] = 0
            for i in zipped_list:
                if rob.r == zipped_list[0][0][1] and rob.c == zipped_list[0][0][1]:
                    zipped_list.remove(i)
                if rob.r == zipped_list[0][1][0] and rob.c == zipped_list[0][1][1]:
                    zipped_list.remove(i)
            #PROBABILITY OF BEEP
            beep = random.random()
            distanceone = BFS(grid, (rob.r, rob.c), (leakCella.r, leakCella.c)) - 1
            power = distanceone * alpha * (-1)
            ...
            total_actions += 1
            #CALCULATIONS BASED ON IS THE BEEP IS HEARD OR NOT(TRUE OR FALSE)
