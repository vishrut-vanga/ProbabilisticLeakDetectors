...
def main():
    ...
    while grid[leakrow][leakcol].status != 'Bot':
        ...
        for i in range(minX, maxX):
            for j in range(minY, maxY):
                #CHECK THE STATUS FOR ALL CELLS INSIDE DETECTION SQUARE
                if grid[i][j].status == 'o' and grid[i][j].leak == False:
                    opencellsinsquare.append([i, j])
                if grid[i][j].leak == True:
                    detection = True
                    opencellsinsquare.append([i, j])
        #INCREMENT TOTAL ACTIONS BY ONE
        total_actions += 1
        if detection == False:
            may_contain_leak = [i for i in may_contain_leak if i not in opencellsinsquare]
        if detection == True:
            may_contain_leak = opencellsinsquare
        #AS OPPOSED TO BOT 1 AND FINDING NEAREST CELL, I MOVE TO RANDOM CELL GIVEN BY NUM VARIABLE UNDERNEATH
        num = random.randint(0, len(may_contain_leak))
        #I MOVE THE BOT TO THE LOCATION GIVEN BY THE RANDOM NUMBER - MAY CONTAIN LEAK[NUM] GIVES A PAIR OF COORDINATES
        grid[rob.r][rob.c].bot_there = False
    ...
    #IT PRINTS TOTAL ACTIONS TAKEN BY BOT TO LEAK CELL
